...
export default function KmTracker(){
  ...
  function clearLocation(w){feedback('tap');(w==='origin'?setOrigin:setDestination)({address:'',lat:null,lng:null});}
        ...
        <div className="km-content-grid">
          ...
          <div className="km-content-left">
            ...
            <section className="km-card">
              ...
              <div className="km-field"><div className="km-field-head"><label className="km-mono km-mono-label"><span className="km-dot km-dot--red"/>· ORIGEM</label>{origin.address&&<button onClick={()=>clearLocation('origin')} className="km-textbtn">CLEAR</button>}</div><textarea value={origin.address} onChange={e=>setOrigin({...origin,address:e.target.value})} placeholder="Endereço de partida" rows={2} className="km-textarea"/><button onClick={()=>capture('origin')} disabled={loading.origin} className="km-btn km-btn--red km-btn--block km-press">{loading.origin?<><Loader2 size={15} className="km-spin"/> BUSCANDO…</>:<><MapPin size={15} strokeWidth={2.4}/> CAPTURAR LOCALIZAÇÃO</>}</button></div>
              ...
              <div className="km-field"><div className="km-field-head"><label className="km-mono km-mono-label"><Flag size={11} strokeWidth={2.4} fill="currentColor"/>· DESTINO</label>{destination.address&&<button onClick={()=>clearLocation('destination')} className="km-textbtn">CLEAR</button>}</div><textarea value={destination.address} onChange={e=>setDestination({...destination,address:e.target.value})} placeholder="Endereço de chegada" rows={2} className="km-textarea"/><button onClick={()=>capture('destination')} disabled={loading.destination} className="km-btn km-btn--ink km-btn--block km-press">{loading.destination?<><Loader2 size={15} className="km-spin"/> BUSCANDO…</>:<><Flag size={15} strokeWidth={2.4}/> CAPTURAR LOCALIZAÇÃO</>}</button></div>
